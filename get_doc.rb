...
@word_list = Array.new
  ...
  class << self
    ...
    def get_list_word(word,list,cat_num)
      @word_list<<word
      docs = Array.new
      begin
        ...
        doc.xpath('//*[@id = "mw-content-text"]//ul//*//a').each do |a|
          if @nest < 1
            if a.content
             puts a.content
             list[a.content] = cat_num
            end
          end
        end
...
#        end
    ...
    def get_doc_from_word(word,cat)
      ...
      begin
        ...
        puts"start parse #{word}"
        doc = Nokogiri::HTML.parse(html,nil,'UTF-8')
        doc.xpath('//*[@id = "mw-content-text"]//ul//li |//*[@id = "mw-content-text"]//p | //*[@id = "mw-content-text"]//span | //*[@id = "mw-content-text"]//dl/dt/*').each do |text|
          puts "						text in doc:#{word}"
          @bc.train(cat,text.content)
        end
=begin
        doc.xpath('//*[@id = "mw-content-text"]//p//a|//*[@id = "mw-content-text"]//dl//dt//a').each do |a|
          if @nest < 1
            @nest+=1 
            get_doc_from_word(a.content,cat) unless @word_list.include?(a.content)
            @nest-=1
...
=end
    ...
    def lean_word(word,cat)
      ...
      begin
        ...
        puts"start parse #{word}"
        doc = Nokogiri::HTML.parse(html,nil,'UTF-8')
        elem = doc.xpath('//*[@id = "mw-content-text"]//ul//li |//*[@id = "mw-content-text"]//p | //*[@id = "mw-content-text"]//span | //*[@id = "mw-content-text"]//dl/dt')
        elem.each do |text|
          puts "						text in doc:#{word}"
          puts text.content
